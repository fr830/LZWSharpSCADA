...
namespace CoreTest
{
    ...
    public class MemberIdentity : IIdentity
    {
        ...
        public int Authenticate(string password)
        {
            ...
            string sql = "SELECT Password FROM Membership WHERE Username='" + _name + "'";
        ...
        {
            string sql = string.Format("DELETE FROM Membership WHERE UserName='{0}';"
               + "INSERT INTO Membership(UserName,Password,ROLE) VALUES('{0}','{1}',{2})",
                ...
                if (reader != null)
                {
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            {
                                sql = "UPDATE Membership SET Password='" + EncodePassword(newPassword) +
