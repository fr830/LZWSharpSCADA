...
namespace DataService
{
    public class ExpressionEval : IDisposable
    {
        ...
        public Func<int> WriteEval(string expression1, string expression2)
        {
            ...
            string[] strNum = expression1.Split('.');
            ...
            if (strNum.Length > 1)
            {
                ...
                if (_server[expression1.ToUpper()] != null)
                {
                    ...
                    var funcbool = dele as Func<bool>;
                    if (funcbool != null)
                        return () => { return WriteTag1(expression1,Convert.ToUInt16(strNum[1]), funcbool()); };
                }
                return null;
            };
            return null;
                    ...
                    switch (expr)
                    {
                        ...
                        case ".":
                            if (number.Count > 1)
                            {
                                Expression right = number.Pop();
                                Expression left = number.Pop();
                                Expression Temp = Expression.LeftShift(Expression.Constant(1), right);
                                Expression Temp1 = Expression.And(left, Temp);
                                number.Push(Expression.Equal(Temp1, Temp));   
        ...
        public int WriteTag1(string tagName, ushort index, bool value)
        {
            ...
            ITag tag = _server[tagName];
                ...
                var val = 1 << index;
                int Value =  value ? (tag.Value.Word | val) : (tag.Value.Word & ~val);
                return tag.Write(Value);
