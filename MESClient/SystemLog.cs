...
namespace MESClient
{
    public class SystemLog : IEvent
    {
        ...
        {
            string sql = string.Format("INSERT INTO LOG_EVENT(EVENTTYPE,SEVERITY,ACTIVETIME,SOURCE,COMMENT) VALUES({0},{1},'{2}','{3}','{4}');",
        ...
        public static SystemLog FindFirstEvent(EventType eventtype, DateTime? firsttime = null, string source = null)
        {
            ...
            string sql = string.Format("SELECT TOP 1 SEVERITY,ACTIVETIME,COMMENT FROM LOG_EVENT WHERE EVENTTYPE={0} {1} {2} ORDER BY ACTIVETIME",
        ...
        public static SystemLog FindLastEvent(EventType eventtype, DateTime? lasttime = null, string source = null)
        {
            ...
            string cond2 = string.IsNullOrEmpty(source) ? "" : " AND SOURCE='" + source + "'";
            string sql = string.Format("SELECT TOP 1 SEVERITY,ACTIVETIME,COMMENT FROM LOG_EVENT WHERE EVENTTYPE={0} {1} {2} ORDER BY ACTIVETIME DESC",
               (int)eventtype, cond1, cond2);
            using (var reader = DataHelper.Instance.ExecuteReader(sql))
            {
                while (reader.Read())
        ...
        public static List<SystemLog> FindEvents(EventType eventtype, string source = null, DateTime? startime = null, DateTime? endtime = null)
        {
            ...
            string sql = string.Format("SELECT SEVERITY,ACTIVETIME,COMMENT FROM LOG_EVENT WHERE EVENTTYPE={0} {1} {2} {3} ORDER BY ACTIVETIME",
